...
summary: 从BlueXP发现内部ONTAP 集群后、您可以打开工作环境来配置和管理存储。 
---
= 管理内部 ONTAP 集群的存储
:hardbreaks:
:allow-uri-read: 
...
从BlueXP发现内部ONTAP 集群后、您可以打开工作环境来配置和管理存储。



== 正在创建卷

通过BlueXP、您可以在现有聚合上创建NFS或CIFS卷。您不能从BlueXP在内部ONTAP 集群上创建新聚合。
...
. 为模板中未硬编码的所有参数添加值。请参见  volumes,正在创建卷 有关部署内部 ONTAP 卷所需完成的所有参数的详细信息。
